...
const Estoque = ({ planoId, auth }) => {
  ...
  const handlePriceChange = (e, index) => {
    ...
    // Garantir que value seja uma string antes de tentar modificar com replace
    if (typeof value === 'string') {
      // Remover caracteres não numéricos, mantendo a vírgula para decimais
  ...
  const handleInputChange = (index, name, value) => {
    ...
    // Assegure que valorUnitario seja sempre uma string
    ...
    // Atualiza o total, convertendo corretamente o valor
    const valorNumerico = parseFloat(updatedItems[index].valorUnitario.replace('R$', '').replace('.', '').replace(',', '.')) || 0;
    updatedItems[index].total = updatedItems[index].quantidade * valorNumerico;
  ...
  const handleSubmit = (e) => {
    ...
    // Remove a formatação de cada item antes de enviar os dados
    const formattedItems = data.items.map(item => ({
      ...item,
      valorUnitario: parseFloat(item.valorUnitario.replace('R$', '').replace('.', '').replace(',', '.')) || 0,
      total: item.quantidade * (parseFloat(item.valorUnitario.replace('R$', '').replace('.', '').replace(',', '.')) || 0),
    }));
